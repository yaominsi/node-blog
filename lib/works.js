...
exports.Works={
  ...
  detail:function(_id_str,callback){
    ...
    MongoClient.connect(app_config.mongodb_url, function(err, db) {
      db.collection('works').findOne({_id:id},function(err,works){
        ...
        if(err) callback(err);
        var item=works;
        db.collection('attaches').find({ref_id:id},{fields:{path:1,path_d:1,filename:1},sort:[['ordering','ascending']]}).toArray(function(err,imgs){
  ...
  save:function(item,callback){
    MongoClient.connect(app_config.mongodb_url, function(err, db) {
      ...
      if(item.imgids){
        db.collection('attaches').findOne({_id:ObjectID(item.imgids[0])},{fields:{path:1}},function(err,result){
